PACKAGE_VERSION=$(node -p -e "try { require('./Nitro-sniper-Death/package.json').version } catch { '0.0.0' }")
LATEST_VERSION=$(curl --silent "https://raw.githubusercontent.com/DeathSpikez/Nitro-sniper-Death/main/package.json" | grep '"version":' | sed -E 's/.*"([^"]+)".*/\1/')
...
function installAll() {
  ...
  git clone https://github.com/DeathSpikez/Nitro-sniper-Death &> /dev/null
